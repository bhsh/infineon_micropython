...
void IfxAsclin_write8_backup(Ifx_ASCLIN *asclin, uint8 *data)
{
    volatile Ifx_ASCLIN_TXDATA *txData = (volatile Ifx_ASCLIN_TXDATA *)&asclin->TXDATA.U;
...
uint8  IfxAsclin_read_python3(Ifx_ASCLIN *asclin)
{
    volatile Ifx_ASCLIN_RXDATA *rxData = (volatile Ifx_ASCLIN_RXDATA *)&asclin->RXDATA.U;
	uint8 data;
	...
	if(asclin->FLAGS.B.RFL == 1)
	{
	  //asclin->FLAGS.B.RFL = 1;
...
void mp_hal_stdout_tx_strn(const char *str, uint32 len) {
    ...
    //int r = write(STDOUT_FILENO, str, len); &MODULE_ASCLIN0
    Ifx_ASCLIN *asclin = &MODULE_ASCLIN0;
    volatile Ifx_ASCLIN_TXDATA *txData = (volatile Ifx_ASCLIN_TXDATA *)&asclin->TXDATA.U;
    //(void)r;
